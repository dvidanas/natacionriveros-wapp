...
function migrate(db: Database.Database) {
  ...
  if (servicesCount.count === 0) {
    ...
    for (const svc of clientConfig.services) {
      ins.run(
        ...
        svc.price > 0 ? String(svc.price) : null,
        svc.duration ?? clientConfig.appointmentDuration ?? 60,
      );
    }
  }
  ...
  try {
    db.exec("ALTER TABLE services ADD COLUMN teacher TEXT");
  } catch { /* ya existe */ }
...
export function setAvailabilityForResource(
  resourceId: number,
  slots: Array<{ day_of_week: number; time_start: string; time_end: string; max_capacity?: number }>
): void {
  const db = getDb();
  db.prepare("DELETE FROM availability_slots WHERE resource_id = ?").run(resourceId);
  const ins = db.prepare("INSERT INTO availability_slots (resource_id, day_of_week, time_start, time_end, max_capacity) VALUES (?, ?, ?, ?, ?)");
  for (const s of slots) ins.run(resourceId, s.day_of_week, s.time_start, s.time_end, s.max_capacity ?? 20);
}

export function getSlotCapacityInfo(resourceId: number, dayOfWeek: number, timeStart: string): { enrolled: number; max_capacity: number } {
  const db = getDb();
  const slot = db
    .prepare<[number, number, string], { max_capacity: number }>(
      "SELECT max_capacity FROM availability_slots WHERE resource_id = ? AND day_of_week = ? AND time_start = ? LIMIT 1"
    )
    .get(resourceId, dayOfWeek, timeStart);
  const enrolled = (db
    .prepare<[number, string], { count: number }>(
      "SELECT COUNT(*) as count FROM appointments WHERE resource_id = ? AND time_start = ? AND status != 'cancelled'"
    )
...
export function getCapacitySummary(): Array<{ resource_name: string; day_of_week: number; time_start: string; enrolled: number; max_capacity: number }> {
  return getDb()
    .prepare<[], { resource_name: string; day_of_week: number; time_start: string; enrolled: number; max_capacity: number }>(
      `SELECT r.name as resource_name, s.day_of_week, s.time_start, s.max_capacity,
        (SELECT COUNT(*) FROM appointments a WHERE a.resource_id = s.resource_id AND a.time_start = s.time_start AND a.status != 'cancelled') as enrolled
       FROM availability_slots s
       JOIN resources r ON s.resource_id = r.id
       WHERE r.active = 1
       ORDER BY r.name, s.day_of_week, s.time_start`
    )
    .all();
}
